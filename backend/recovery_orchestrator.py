...
class RecoveryOrchestrator:
        ...
        try:
            ...
            if open_duration > threshold:
                if self._check_cooldown("circuit_restart"):
                    ...
                    await self._execute_action("circuit_restart", f"open_duration={open_duration}s")
                else:
        ...
        try:
            ...
            if failures_1h > threshold:
                if self._check_cooldown("cache_purge"):
                    ...
                    await self._execute_action("cache_purge", f"failures_1h={failures_1h}")
                else:
                    cooldown_remaining = self._get_cooldown_remaining("cache_purge")
                    logger.debug(f"Cache purge in cooldown ({cooldown_remaining:.0f}s remaining)")
        except Exception as e:
        ...
        try:
            ...
            if failed_components >= threshold:
                if self._check_cooldown("safe_mode"):
                    ...
                    await self._execute_action("safe_mode", f"failed_components={failed_components}")
                else:
                    cooldown_remaining = self._get_cooldown_remaining("safe_mode")
                    logger.debug(f"Safe mode in cooldown ({cooldown_remaining:.0f}s remaining)")
        except Exception as e:
    ...
    async def _execute_action(self, action_type: str, reason: str = ""):
        ...
        try:
            logger.info(f"▶️  Executing recovery action: {action_type} (reason: {reason})")
            ...
            logger.info(f"✅ Recovery action succeeded: {action_type} ({action.duration_seconds:.1f}s)")
