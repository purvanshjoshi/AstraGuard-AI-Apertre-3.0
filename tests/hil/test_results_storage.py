...
class TestSaveScenarioResult:
    ...
    def test_save_scenario_result_with_metadata(self, temp_storage):
        """Test that saved result includes required metadata."""
        result = {"success": False, "error": "Test error"}
        
        filepath = temp_storage.save_scenario_result("metadata_test", result)
        saved_data = json.loads(Path(filepath).read_text())
    ...
    def test_save_scenario_result_filename_format(self, temp_storage):
        ...
        with patch('astraguard.hil.results.storage.datetime') as mock_datetime:
            ...
            filepath = temp_storage.save_scenario_result("format_test", result)
            
            expected_filename = "format_test_20240101_120000.json"
            assert Path(filepath).name == expected_filename

    def test_save_scenario_result_complex_data(self, temp_storage):
        ...
        filepath = temp_storage.save_scenario_result("complex_test", result)
        saved_data = json.loads(Path(filepath).read_text())
    ...
    def test_save_scenario_result_special_characters(self, temp_storage):
        ...
        filepath = temp_storage.save_scenario_result(scenario_name, result)
        
        assert scenario_name in filepath
        assert Path(filepath).exists()

    def test_save_scenario_result_json_serialization(self, temp_storage):
        ...
        filepath = temp_storage.save_scenario_result("serialization_test", result)
        saved_data = json.loads(Path(filepath).read_text())
...
class TestGetScenarioResults:
    ...
    def test_get_scenario_results_nonexistent(self, temp_storage_with_data):
        """Test retrieving results for non-existent scenario."""
        results = temp_storage_with_data.get_scenario_results("nonexistent_scenario")
        
        assert results == []

    def test_get_scenario_results_corrupted_file(self):
        """Test handling of corrupted JSON files."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            
            # Create a corrupted JSON file
            corrupted_file = Path(temp_dir) / "test_scenario_20240101_120000.json"
            corrupted_file.write_text("invalid json content {")
            
            # Create a valid file
            valid_file = Path(temp_dir) / "test_scenario_20240102_120000.json"
            valid_data = {
                "scenario_name": "test_scenario",
            ...
            results = storage.get_scenario_results("test_scenario")
            assert len(results) == 1
            assert results[0]["success"] is True

    def test_get_scenario_results_empty_directory(self):
        """Test retrieving results from empty directory."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            results = storage.get_scenario_results("any_scenario")
...
class TestGetRecentCampaigns:
    ...
    @pytest.fixture
    def temp_storage_with_campaigns(self):
    ...
    def test_get_recent_campaigns_empty(self):
        """Test retrieving campaigns from empty directory."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            campaigns = storage.get_recent_campaigns()
            
            assert campaigns == []

    def test_get_recent_campaigns_corrupted_file(self):
        """Test handling of corrupted campaign files."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            
            # Create corrupted file
            corrupted_file = Path(temp_dir) / "campaign_20240101_120000.json"
            corrupted_file.write_text("invalid json")
            
            # Create valid file
            valid_file = Path(temp_dir) / "campaign_20240102_120000.json"
            valid_data = {
                "campaign_id": "20240102_120000",
                "total_scenarios": 1,
                "passed": 1
...
class TestGetCampaignSummary:
    ...
    @pytest.fixture
    def temp_storage_with_campaign(self):
    ...
    def test_get_campaign_summary_corrupted_file(self):
...
class TestGetResultStatistics:
    ...
    @pytest.fixture
    def temp_storage_with_stats_data(self):
    ...
    def test_get_result_statistics_empty(self):
    ...
    def test_get_result_statistics_missing_fields(self):
...
class TestClearResults:
    ...
    def test_clear_results_basic(self):
        """Test basic result clearing functionality."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            
            # Create some test files
            old_file = Path(temp_dir) / "old_result.json"
            recent_file = Path(temp_dir) / "recent_result.json"
            ...
            deleted_count = storage.clear_results(older_than_days=30)
            
            assert deleted_count == 1
            assert not old_file.exists()
            assert recent_file.exists()

    def test_clear_results_no_files(self):
    ...
    def test_clear_results_all_recent(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create recent files
            for i in range(3):
                test_file = Path(temp_dir) / f"recent_{i}.json"
                test_file.write_text(f'{{"test": {i}}}')
            
            deleted_count = storage.clear_results(older_than_days=30)
            
            assert deleted_count == 0
            assert len(list(Path(temp_dir).glob("*.json"))) == 3

    def test_clear_results_custom_age(self):
        """Test clearing with custom age threshold."""
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = ResultStorage(results_dir=temp_dir)
            
            test_file = Path(temp_dir) / "test_result.json"
            ...
            deleted_count = storage.clear_results(older_than_days=3)
...
class TestIntegrationScenarios:
    ...
    def test_full_workflow_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, (scenario, result) in enumerate(zip(scenarios, results)):
                ...
                path = storage.save_scenario_result(scenario, result)
            ...
            scenario_a_results = storage.get_scenario_results("scenario_a")
            assert len(scenario_a_results) == 2
            
            # Retrieve results for scenario_b (should have 1 result)
            scenario_b_results = storage.get_scenario_results("scenario_b")
            ...
            deleted = storage.clear_results(older_than_days=1)
            assert deleted == 0
    ...
    def test_concurrent_access_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(5):
                ...
                path = storage.save_scenario_result("concurrent_test", result)
                results.append(path)
            ...
            retrieved = storage.get_scenario_results("concurrent_test")
    ...
    def test_large_result_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path = storage.save_scenario_result("large_test", large_result)
            assert Path(path).exists()
            
            # Verify file size is reasonable (should be compressed JSON)
            file_size = Path(path).stat().st_size
            assert file_size > 1000  # Should be substantial
            
            # Retrieve and verify data integrity
            retrieved = storage.get_scenario_results("large_test")
